...
const pages = dv.pages('"1_Whiskeys"')
	.filter(p => p.Type && p.Type.toLowerCase().includes("bourbon") && p.Inventory > 0)
...
const pages = dv.pages('"1_Whiskeys"')
	.filter(p => p.Type && !p.Type.toLowerCase().includes("bourbon") && !["Rye Whiskey", "Tennessee Whiskey"].includes(p.Type) && p.Inventory > 0)
	.sort((a, b) => (b["Total Score"] ?? 0) - (a["Total Score"] ?? 0)); // Descending
